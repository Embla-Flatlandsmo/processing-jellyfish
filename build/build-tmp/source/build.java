...
class Tentacles {
  ...
  Tentacles(PVector origin_, int count_, float length_, float tentacleThickness_, float strokeThickness_, float hue) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      int c = PApplet.parseInt(color(random(hue-20, hue+20), random(10,25), random(75,90)));
      colors.add(c);

      if (prob < 0.3f) { //short tentacles
          Tentacle currentTentacle = new Tentacle(new PVector(x,y), PApplet.parseInt(length*random(0.1f,0.3f)));
          currentTentacle.makePoints();
          tentacles.add(currentTentacle);
      } else if (prob < 0.9f) { // (if 0.3<prob<0.9): medium tentacles
          Tentacle currentTentacle = new Tentacle(new PVector(x,y), PApplet.parseInt(length*random(0.4f,0.6f)));
          currentTentacle.makePoints();
          tentacles.add(currentTentacle);
      } else { //long tentacles
          Tentacle currentTentacle = new Tentacle(new PVector(x,y), PApplet.parseInt(length*random(0.6f,1)));
          currentTentacle.makePoints();
          tentacles.add(currentTentacle);
      }
    }
...
class Tentacle {
  ...
  public void makePoints() {
    for (int i = 0; i < length; i++) {
      ...
      float randomRadian = noise((0.01f*locations.get(i).x)*pow(0.99f,i), (locations.get(i).y)*0.01f*pow(0.99f,i))*4*PI;
      float randomMagnitude = noise((locations.get(i).x)*0.01f*pow(0.99f,i), (locations.get(i).y)*0.01f*pow(0.99f,i));
  ...
  public void display(float tentacleThickness, float strokeThickness, int col) {
    stroke(0xff836DAA);
  ...
  public void settings() {  size(600,600,P2D);  smooth(8); }
